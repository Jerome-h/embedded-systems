...
def run():
    ...
    payload = json.dumps({'name':'mdeded', 'time':clocktime, 'temp':test1.readtemp()})
    print(payload)
...
def log(interval):
    ...
    while True:
        ...
        #message = {time: test1.readtemp()}
        temp = test1.readtemp()
        if temp > old+0.3 or temp < old-0.3 :
            payload = json.dumps({'name':'mdeded', 'time':clocktime, 'temp':temp})
            print(payload)
            client.publish('/esys/mdeded/data/',bytes(payload,'utf-8'))
            old = temp
        time.sleep(interval)
